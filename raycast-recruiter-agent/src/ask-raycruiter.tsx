...
export default function Command() {
  ...
  const onSubmit = async (queryOverride?: string | any) => {
    const finalQuery =
      typeof queryOverride === "string" ? queryOverride : searchText;
    ...
    try {
        ...
        functionDeclarations: [
          {
            name: "search_deals",
            description:
              "Search the local deals dataset for active pipelines, companies, and deals by name or keyword.",
            parameters: {
              ...
              properties: {
                keywords: {
                  ...
                  description:
                    "A list of specific companies, person names, or inferred tech industry names generated intelligently by you based on the user's intent to query the local ATS (e.g. ['tudigo', 'karmen', 'fintech', 'john']).",
                },
                orderBy: {
                  type: SchemaType.STRING,
                  description:
                    "Optional sorting request (e.g. 'date' to order by closest due date, or 'value' for deal size).",
                },
              },
              required: ["keywords"],
            },
          },
          {
            ...
            description:
              "Generate and auto-pipeline highly qualified B2B deals sourced via Mantiks.",
            parameters: {
              type: SchemaType.OBJECT,
              properties: {
                industry: {
                  type: SchemaType.STRING,
                  description: "The industry to source leads in.",
                },
                amount: {
                  type: SchemaType.NUMBER,
                  description: "Number of deals to generate.",
                },
              },
      ...
      const model = genAI.getGenerativeModel({
        ...
        systemInstruction:
          "You are an expert recruiter and pipeline manager. When asked about existing deals, use 'search_deals'. IF the user asks to explicitly 'generate', 'source', or 'find brand new deals via Mantiks', ALWAYS call 'generate_mantiks_deals'. Always format output nicely using Markdown.",
      });
      ...
      if (calls && calls.length > 0) {
        ...
        if (call.name === "search_deals") {
          console.log(
            "🤖 [RAG INTERCEPT] AI invoked 'search_deals'. Arguments:",
            JSON.stringify(call.args),
          );

          const rawKeywords = (call.args as any).keywords;
          const orderBy =
            typeof (call.args as any).orderBy === "string"
              ? (call.args as any).orderBy.toLowerCase()
              : null;
          ...
          if (keywords.length === 0) {
            ...
            matches = (dealsData as any[]).filter((d) => {
          ...
          if (orderBy === "date") {
            matches.sort(
              (a, b) =>
                new Date(a.next_activity_date || 0).getTime() -
                new Date(b.next_activity_date || 0).getTime(),
            );
          ...
          console.log(
            `✅ [RAG RETURN] Sending ${matches.length} matches back to AI. Example deal title:`,
            matches[0]?.title || "None",
          );
          ...
          console.log(
            "🤖 [MANTIKS INTERCEPT] AI invoked 'generate_mantiks_deals'. Arguments:",
            JSON.stringify(call.args),
          );
          ...
          console.log(
            `✨ [MANTIKS RETURN] Generated ${newDeals.length} deals. Sending back to AI.`,
          );
      ...
      console.log(
        "📝 [AI TEXT RESPONSE] Raw string output intercepted from Gemini:",
        responseText,
      );
      ...
      if (responseText.includes("FUNCTION_CALL: SOURCE_LEADS")) {
        ...
        setTimeout(() => {
          ...
          setMessages((prev) => [
            {
              role: "system",
              text: `(System) Successfully found ${leads.length} prospect(s) via Mantiks API.`,
            },
            ...prev,
          ]);
          setIsLoading(false);
      ...
      setMessages((prev) => [
        {
          role: "system",
          text: `API Error: ${error?.message || String(error)}`,
        },
        ...prev,
      ]);
    }
  ...
  return (
    <List
      ...
      isShowingDetail={
        (sourcedLeads !== null && sourcedLeads.length > 0) ||
        (foundDeals !== null && foundDeals.length > 0) ||
        (generatedDeals !== null && generatedDeals.length > 0)
      }
    >
              ...
              actions={
                <ActionPanel>
                  <Action
                    title="Ask AI"
                    icon={Icon.Wand}
                    onAction={() => onSubmit("Sort active deals by due date")}
                  />
                </ActionPanel>
              }
            />
              ...
              actions={
                <ActionPanel>
                  <Action
                    ...
                    icon={Icon.Wand}
                    onAction={() =>
                      onSubmit("Generate 5 new Fintech leads via Mantiks")
                    }
                  />
                </ActionPanel>
              }
              ...
              actions={
                <ActionPanel>
                  <Action
                    ...
                    icon={Icon.Wand}
                    onAction={() =>
                      onSubmit("Find the highest value deals in my pipeline")
                    }
                  />
                </ActionPanel>
              }
              ...
              actions={
                <ActionPanel>
                  <Action
                    ...
                    icon={Icon.Wand}
                    onAction={() =>
                      onSubmit("Look up the status of the Karmen deal")
                    }
                  />
                </ActionPanel>
              }
            ...
            <List.Item
              ...
              accessories={[
                ...
                deal.next_activity_date
                  ? {
                      date: new Date(deal.next_activity_date),
                      tooltip: "Next Activity Date",
                    }
                  : {},
                  ...
                  metadata={
                    <List.Item.Detail.Metadata>
                      ...
                      <List.Item.Detail.Metadata.Label
                        ...
                        text={deal.person_id?.name || "N/A"}
                        icon={Icon.Person}
                      />
                      {deal.person_id?.email?.[0]?.value && (
                        <List.Item.Detail.Metadata.Label
                          title="Email"
                          text={deal.person_id.email[0].value}
                          icon={Icon.Envelope}
                        />
                      )}
                      {deal.person_id?.phone?.[0]?.value && (
                        <List.Item.Detail.Metadata.Label
                          title="Phone"
                          text={deal.person_id.phone[0].value}
                          icon={Icon.Phone}
                        />
                      )}
                    </List.Item.Detail.Metadata>
              ...
              actions={
                <ActionPanel>
                  ...
                  {deal.person_id?.phone?.[0]?.value && (
                    <Action.CopyToClipboard
                      title="Copy Phone"
                      content={deal.person_id.phone[0].value}
                    />
                  )}
                </ActionPanel>
              }
            ...
            <List.Item
              ...
              actions={
                <ActionPanel>
                  ...
                  <Action.OpenInBrowser
                    title="Power Dial"
                    url={`tel:${lead.phone.replace(/[^0-9+]/g, "")}`}
                    icon={Icon.Phone}
                  />
                  <Action.CopyToClipboard
                    title="Copy Email"
          ...
          {generatedDeals.map((deal) => (
            <List.Item
              ...
              accessories={[
                {
                  text: deal.value ? `$${deal.value.toLocaleString()}` : "",
                  tooltip: "Deal Value (Mantiks Estimate)",
                },
                deal.next_activity_date
                  ? {
                      date: new Date(deal.next_activity_date),
                      tooltip: "Auto-Scheduled Kickoff",
                    }
                  : {},
              ].filter((a) => Object.keys(a).length > 0)}
              detail={
