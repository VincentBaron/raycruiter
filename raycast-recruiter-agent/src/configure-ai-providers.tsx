    ...
    return (
        <List isLoading={loading} searchBarPlaceholder="Search Providers...">
            <List.Item
                ...
                icon={Icon.Stars}
                accessories={[{ icon: geminiKey ? Icon.CheckCircle : Icon.Circle, tooltip: geminiKey ? "Configured" : "Not Configured" }]}
                actions={
                    <ActionPanel>
                        <Action.Push title="Set API Key" target={<EditProviderKey providerName="Google Gemini" storageKey="GEMINI_API_KEY" onSave={loadKeys} />} />
                        {geminiKey && <Action title="Clear Key" onAction={async () => { await LocalStorage.removeItem("GEMINI_API_KEY"); loadKeys(); showToast(Toast.Style.Success, "Cleared"); }} />}
                    </ActionPanel>
                }
            />
            <List.Item
                ...
                icon={Icon.Globe}
                accessories={[{ icon: openAiKey ? Icon.CheckCircle : Icon.Circle, tooltip: openAiKey ? "Configured" : "Not Configured" }]}
                actions={
                    <ActionPanel>
                        <Action.Push title="Set API Key" target={<EditProviderKey providerName="OpenAI" storageKey="OPENAI_API_KEY" onSave={loadKeys} />} />
                        {openAiKey && <Action title="Clear Key" onAction={async () => { await LocalStorage.removeItem("OPENAI_API_KEY"); loadKeys(); showToast(Toast.Style.Success, "Cleared"); }} />}
                    </ActionPanel>
                }
            />
        </List>
