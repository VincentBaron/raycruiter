...
(() => {
  ...
  var import_api = __require("@raycast/api");
  function Persons() {
    const [searchText, setSearchText] = (0, import_react.useState)("");
    const [isLoading, setIsLoading] = (0, import_react.useState)(false);
    const persons = (0, import_react.useMemo)(() => {
      try {
        const data = JSON.parse(__require("fs").readFileSync("/Users/vincentbaron/raycruiter/persons_cache_2026-03-22_12-18-43.json", "utf8"));
        return data;
      } catch (e) {
        return [];
      }
    }, []);
    const filteredPersons = (0, import_react.useMemo)(() => {
      if (!searchText) return persons.slice(0, 100);
      return persons.filter(
        (p) => p.name?.toLowerCase().includes(searchText.toLowerCase()) || p.emails?.some((e) => e.value.toLowerCase().includes(searchText.toLowerCase())) || p.job_title?.toLowerCase().includes(searchText.toLowerCase())
      ).slice(0, 50);
    }, [searchText, persons]);
    return /* @__PURE__ */ import_react.default.createElement(
      ...
      filteredPersons.map((person) => {
        const primaryEmail = person.emails?.find((e) => e.primary)?.value || person.emails?.[0]?.value || "No Email";
        const primaryPhone = person.phones?.find((p) => p.primary)?.value || person.phones?.[0]?.value || "No Phone";
        let orgName = "No Org";
        ...
        return /* @__PURE__ */ import_react.default.createElement(
          import_api.List.Item,
          {
            ...
            icon: import_api.Icon.PersonCircle,
            accessories: [
              { text: primaryPhone, icon: import_api.Icon.Phone },
              { text: primaryEmail, icon: import_api.Icon.Envelope }
            ],
            actions: /* @__PURE__ */ import_react.default.createElement(import_api.ActionPanel, null, /* @__PURE__ */ import_react.default.createElement(import_api.ActionPanel.Section, { title: "Next Steps in Pipeline" }, /* @__PURE__ */ import_react.default.createElement(
              import_api.Action,
              {
                title: "Push & Convert to Deal",
                icon: import_api.Icon.ArrowRightCircle,
                onAction: () => (0, import_api.showToast)({ style: import_api.Toast.Style.Success, title: "Deal Triggered!", message: "Pipeline logic: Deal filled -> Job generated -> MultiDiffusion triggered -> Organisation Created." })
              }
