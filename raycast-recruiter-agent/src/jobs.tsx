...
const activeJobsData = JSON.parse(
  fs.readFileSync(
    "/Users/vincentbaron/raycruiter/raycast-recruiter-agent/active_jobs.json",
    "utf8",
  ),
);
...
export default function JobsCommand() {
  ...
  const actualJobs = activeJobsData.items.map((item: any, index: number) => {
    ...
    return {
      ...
      location:
        item.vacancy.addressFormatted ||
        item.vacancy.address?.locality ||
        "Remote",
  ...
  const filteredJobs = actualJobs.filter((j: any) => {
    ...
    const charSum =
      j.title.charCodeAt(0) + j.title.charCodeAt(j.title.length - 1);
    let status = "active";
            ...
            actions={
              <ActionPanel>
                <Action.CopyToClipboard
                  title="Copy Job Link"
                  content={`https://raycruiter.app/jobs/${job.id}`}
                />
              </ActionPanel>
            }
          />
