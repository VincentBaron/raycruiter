    ...
    async function load() {
      ...
      if (key) {
        ...
        showToast({
          ...
          message: "Set Kalent API Key in Integrations Hub",
        });
      }
    }
    load();
    ...
    >
      {candidates.length === 0 && !loading && (
        <List.EmptyView
          ...
          description="Try a broader search or verify your Kalent API token."
          icon={Icon.Person}
        />
      )}
      <List.Section
        title={`Kalent Candidates ${query ? `for "${query}"` : ""}`}
      >
