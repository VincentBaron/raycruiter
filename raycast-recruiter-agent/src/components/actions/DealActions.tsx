...
export function DealActions({ deal }: { deal: any }) {
  ...
  const assignMember = async (memberName: string) => {
    const updated = deals.map((d) =>
      d.id === deal.id ? { ...d, assignee: memberName } : d,
    );
    setDeals(updated);
    await showToast({
      style: Toast.Style.Success,
      title: `Assigned to ${memberName}`,
    });
  };

  const changeStatus = async () => {
    const nextStatus =
      deal.status === "open" ? "won" : deal.status === "won" ? "lost" : "open";
    const updated = deals.map((d) =>
    ...
    await showToast({
      style: Toast.Style.Success,
      title: `Deal Status: ${nextStatus.toUpperCase()}`,
    });
  };
    ...
    <ActionPanel>
      <ActionPanel.Section>
        <Action.Push
          title="Open Details"
          icon={Icon.Sidebar}
          target={<EntityDetail type="Deal" data={deal} />}
        />
        <Action.Push
          title="Create Job"
          icon={Icon.Plus}
          target={<CreateJobForm dealId={deal.id} />}
        ...
        >
          ...
          <Action
            title="John Smith"
            onAction={() => assignMember("John Smith")}
          />
          <Action
            title="Alex Recruiter"
            onAction={() => assignMember("Alex Recruiter")}
          />
        </ActionPanel.Submenu>
        ...
        <Action
          title="Edit Deal"
          icon={Icon.Pencil}
          onAction={() => showToast({ title: "Opening Edit Form" })}
