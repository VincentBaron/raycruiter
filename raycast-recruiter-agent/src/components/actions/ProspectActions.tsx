    ...
    <ActionPanel>
      <ActionPanel.Section>
        <Action.Push
          title="Open Details"
          icon={Icon.Sidebar}
          target={<EntityDetail type="Prospect" data={prospect} />}
        />
        <Action
          title="Convert to Client"
          icon={Icon.PersonCircle}
          onAction={convertToDeal}
          shortcut={{ modifiers: ["cmd", "shift"], key: "c" }}
        />
        <Action
          title="Send Call Recap"
          icon={Icon.Envelope}
          onAction={() => showToast({ title: "Recap Generated via AI" })}
          shortcut={{ modifiers: ["cmd", "shift"], key: "r" }}
        />
        <Action
          title="Add Note"
          icon={Icon.Document}
          onAction={() => showToast({ title: "Add Note" })}
          shortcut={{ modifiers: ["cmd"], key: "n" }}
        />
        <Action
          title="Edit Prospect"
          icon={Icon.Pencil}
          onAction={() => showToast({ title: "Edit Form" })}
          shortcut={{ modifiers: ["cmd"], key: "e" }}
        />
      </ActionPanel.Section>
      <ActionPanel.Section>
        {prospect.email && (
          <Action.CopyToClipboard title="Copy Email" content={prospect.email} />
        )}
