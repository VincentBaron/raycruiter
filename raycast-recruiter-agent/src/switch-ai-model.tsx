...
export default function SwitchAiModel() {
  ...
  async function handleSelect(id: string) {
    ...
    showToast({
      style: Toast.Style.Success,
      title: "Model Changed",
      message: `Active Model is now ${id}`,
    });
  }
  ...
  return (
    <List searchBarPlaceholder="Search Models...">
      {SUPPORTED_MODELS.map((m) => {
        const isReady =
          (m.type === "gemini" && hasGemini) ||
          (m.type === "openai" && hasOpenAi);
        return (
          <List.Item
            key={m.id}
            title={m.name}
            ...
            accessories={[
              ...(!isReady
                ? [{ icon: Icon.LockDisabled, tooltip: "Missing API Key" }]
                : []),
              ...(activeModel === m.id
                ? [{ icon: Icon.CheckCircle, tooltip: "Active Model" }]
                : []),
            ]}
            actions={
              <ActionPanel>
