    ...
    <List navigationTitle="Multi-Diffusion Boards">
      {items.map((board) => (
        <List.Item
          key={board.id}
          title={board.name}
          icon={board.active ? { source: Icon.CheckCircle, tintColor: "green" } : { source: Icon.Circle, tintColor: "gray" }}
          accessories={[{ text: board.active ? "Active" : "Inactive" }]}
          actions={
            <ActionPanel>
              <Action title={board.active ? "Disable Board" : "Enable Board"} icon={Icon.Power} onAction={() => toggleBoard(board.id)} />
              <Action.OpenInBrowser title="Open Board Website" url={`https://www.${board.id}.com`} />
            </ActionPanel>
          }
        />
      ))}
