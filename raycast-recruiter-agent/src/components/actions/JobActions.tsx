...
export function JobActions({ job }: { job: any }) {
  ...
  const sourceCandidates = async () => {
    ...
    await showToast({
      ...
      title: "Sourced 1 Candidate via Jemmo AI",
    });
  };
    ...
    <ActionPanel>
      <ActionPanel.Section>
        <Action.Push
          title="Open Details"
          icon={Icon.Sidebar}
          target={<EntityDetail type="Job" data={job} />}
        />
        <Action.Push
          title="View Applicants"
          icon={Icon.TwoPeople}
          target={<ViewApplicantsList jobId={job.id} />}
          shortcut={{ modifiers: ["cmd", "shift"], key: "v" }}
        />
        <Action
          title="Source Candidates"
          icon={Icon.MagnifyingGlass}
          onAction={sourceCandidates}
          shortcut={{ modifiers: ["cmd", "shift"], key: "g" }}
        />
        <Action
          title="Toggle Multi-Diffusion"
          icon={Icon.ArrowNe}
          onAction={() => showToast({ title: "Multi-Diffusion Toggled" })}
