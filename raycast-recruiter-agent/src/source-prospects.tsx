...
export default function SourceProspects() {
  ...
  async function pushToPipedrive(lead: MantiksLead, stage: string) {
    ...
    const pNotes = await LocalStorage.getItem<string>(
      `notes_prospect_${prospectEmail}`,
    );
    if (pNotes) await LocalStorage.setItem(`notes_deal_${newDealId}`, pNotes);

    const pActs = await LocalStorage.getItem<string>(
      `activities_prospect_${prospectEmail}`,
    );
    if (pActs)
      await LocalStorage.setItem(`activities_deal_${newDealId}`, pActs);
  ...
  async function pushAndDial(lead: MantiksLead) {
    ...
    await showToast({
      style: Toast.Style.Success,
      title: "Person Created! Jumping to Dialer...",
    });
    setIsSyncing(false);
    ...
    push(
      <PowerDialer initialSearchText={fullName} injectedPerson={injected} />,
    );
  }
  ...
  return (
    <List
      ...
      searchBarAccessory={
        <List.Dropdown
          tooltip="Select Prospect Sourcing Pipeline"
          value={selectedPipeline}
          onChange={setSelectedPipeline}
        >
          <List.Dropdown.Item
            title="All Sourced Pipelines"
            value="all"
            icon={Icon.List}
          />
          <List.Dropdown.Item
            title="Tech Sourcing - June"
            value="1"
            icon={Icon.Terminal}
          />
          <List.Dropdown.Item
            title="Sales Sourcing EMEA"
            value="2"
            icon={Icon.Globe}
          />
        </List.Dropdown>
                  ...
                  <ActionPanel.Section title="Prospect Activity">
                    <Action
                      ...
                      onAction={() =>
                        push(
                          <LogNoteForm
                            entityId={`prospect_${lead.lead_email}`}
