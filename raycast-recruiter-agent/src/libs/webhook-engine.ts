...
export async function sendWebhook(
  ...
  try {
    const webhookUrlsStr =
      (await LocalStorage.getItem<string>("custom_webhooks")) || "{}";
    ...
    if (!targetUrl) {
      console.log(
        `No webhook configured for event: ${event}. Storing purely locally.`,
      );
